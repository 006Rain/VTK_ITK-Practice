/*
Description:
This is the demonstration of how to construct and display
geometric objects using the classes derived from vtkCell3D.
For each object we specify the points and cell Ids.

From this we create an unstructured grid. In some cases a vtkCellArray
is used and the result is add to the unstructured grid.

Also demonstrates is the use of vectors to hold the unstructured grid,
mappers, actors and renderers.
...
namespace
{
	//These function return a vtkUnstructured grid corresponding 
	//to the object.
...
namespace
{
	vtkSmartPointer<vtkUnstructuredGrid> MakeHexagonalPrism()
	{
		...
		{
			hexagonalPrism->GetPointIds()->SetId( n, n );
		}
