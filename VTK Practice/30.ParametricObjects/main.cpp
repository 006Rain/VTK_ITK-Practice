...
int main()
{
	...
	/*
	Uncomment one of the following and ensure the matching include(above) is uncommented.
	*/
	...
	rendWindow->SetWindowName( "Parametric Objects" );
	rendWindow->SetSize( 600, 400 );
	rendWindow->SetPosition( 500, 150 );
	rendWindow->AddRenderer( renderer );

	vtkSmartPointer<vtkRenderWindowInteractor> renderWindowInteract = vtkSmartPointer<vtkRenderWindowInteractor>::New();
	renderWindowInteract->SetRenderWindow( rendWindow );

	renderer->AddActor( actor );
	renderer->SetBackground( colors->GetColor3d( "BkgColor" ).GetData() );

	rendWindow->Render();
	renderWindowInteract->Start();
